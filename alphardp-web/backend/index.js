...
app.get('/api/vps/list', authenticate, async (req, res) => {
    try {
        ...
        // Filter out hidden servers and admin servers for non-admins
        if (!isAdmin(req.user)) {
            data = data.filter(vps => !vps.name.startsWith('[HIDDEN] ') && vps.server_type !== 'Admin');
        }
        ...
        const safeData = data.map(vps => {
            ...
            return {
                ...
                server_type: vps.server_type || 'Public',
                used_minutes: vps.used_minutes || 0,
...
app.get('/api/vps/setup/stream/:id', authenticate, async (req, res) => {
    try {
        ...
        // Cleanup previous session data (Skip if Admin server)
        if (vps.server_type === 'Admin') {
            res.write('data: [SYSTEM] Admin server detected. Skipping cleanup to preserve data.\n\n');
        } else {
...
app.post('/api/admin/vps', authenticate, adminOnly, async (req, res) => {
    try {
        const { name, codespace_name, github_token, ram_gb, cpu_cores, os, rdp_username, rdp_password, server_type } = req.body;
        const { data, error } = await supabase.from('vps_instances').insert([
            { name, codespace_name, github_token, ram_gb, cpu_cores, os, rdp_username, rdp_password, server_type: server_type || 'Public', status: 'Shutdown' }
        ]).select().single();
