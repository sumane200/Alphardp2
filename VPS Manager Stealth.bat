...
title GitHub VPS Manager (Fast)

REM ============================================================
REM GitHub VPS Manager - OPTIMIZED
REM
REM vps_data.txt format:
REM VPS_NAME,GITHUB_TOKEN,CODESPACE_NAME,RDP_USER,RDP_PASS,RDP_HOST
REM
REM OPTIMIZATIONS vs original:
REM  - Steps 2-5 merged into a SINGLE SSH call (saves ~15-20s)
REM  - Polling interval reduced: 2s -> 1s
REM  - SSH wait attempts: 30x2s -> 15x1s  (max 15s vs 60s)
REM  - Pinggy poll attempts: 30x1s (max 30s)
REM  - Tunnel: Pinggy SSH reverse tunnel (replaces ngrok, no binary needed)
REM ============================================================
...
REM ============================================================
REM ACTIVATE VPS  [OPTIMIZED - 3 steps instead of 6]
REM ============================================================
...
REM ============================================================
REM [1/3] START CODESPACE + WAIT FOR SSH
REM ============================================================

echo [1/3] Starting Codespace...
echo.
...
REM OPTIMIZED: 15 attempts x 1s = max 15s  (original was 30x2=60s)
...
REM ============================================================
REM [2/3] ONE SSH CALL: xrdp restart + start Pinggy tunnel
REM       Pinggy = free SSH reverse tunnel, no binary needed!
REM ============================================================

echo [2/3] Starting services (xrdp + Pinggy tunnel)...
echo.

REM Restart xrdp, kill any old Pinggy SSH tunnel, start a fresh detached one.
gh cs ssh -c "%TARGET_CS%" -- "sudo service xrdp restart >/dev/null 2>&1; sleep 2; service xrdp status >/dev/null 2>&1 || sudo service xrdp start >/dev/null 2>&1" 2>nul

echo Applying stealth cloaking...
gh cs cp "%~dp0cloak.sh" "remote:cloak.sh" -c "%TARGET_CS%" >nul 2>&1
gh cs ssh -c "%TARGET_CS%" -- "bash ~/cloak.sh" >nul 2>&1
...
echo xrdp running. Cloaking applied. Pinggy tunnel launched.
echo.


REM ============================================================
REM [3/3] WAIT FOR PINGGY TUNNEL
REM       Single SSH call - polls log file on Codespace side
REM ============================================================

echo [3/3] Waiting for Pinggy tunnel address (please wait)...
echo.
...
echo Stopping Pinggy tunnel and Codespace...
